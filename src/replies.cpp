...
IRCReply    replay_list( int code )
{
    ...
    //ERROR REPLIES
    rep.push_back((IRCReply){401, "ERR_NOSUCHNICK", "<nickname> :No such nick/channel", "Used to indicate the nickname parameter supplied to a command is currently unused."});
    ...
    rep.push_back((IRCReply){432, "ERR_ERRONEUSNICKNAME", "<nick> :Erroneus nickname", "Returned after receiving a NICK message which contains characters which do not fall in the defined set. See section x.x.x for details on valid nicknames."});
